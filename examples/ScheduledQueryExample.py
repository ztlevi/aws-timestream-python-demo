...
class ScheduledQueryExample:
    ...
    ERROR_BUCKET_NAME = "scheduledquerysamplerrorbucket" + "".join(
    ...
    def create_scheduled_query_helper(
        self, topic_arn, role_arn, query, target_configuration
    ):
        print("\nCreating Scheduled Query")
        schedule_configuration = {"ScheduleExpression": "cron(0/2 * * * ? *)"}
        notification_configuration = {"SnsConfiguration": {"TopicArn": topic_arn}}
        error_report_configuration = {
            "S3Configuration": {"BucketName": self.s3_error_bucket}
        }

        try:
            create_scheduled_query_response = self.query_client.create_scheduled_query(
                Name=self.SQ_NAME,
                QueryString=query,
            ...
            print(
                "Successfully created scheduled query : ",
    ...
    def create_valid_scheduled_query(self, topic_arn, role_arn):
        ...
        return self.create_scheduled_query_helper(
            topic_arn, role_arn, self.valid_query, target_configuration
        )
    ...
    def create_invalid_scheduled_query(self, topic_arn, role_arn):
        ...
        return self.create_scheduled_query_helper(
            topic_arn, role_arn, self.invalid_query, target_configuration
        )
    ...
    def list_scheduled_queries(self):
        ...
        try:
            ...
            while next_token:
                response = self.query_client.list_scheduled_queries(
    ...
    def execute_scheduled_query(self, scheduled_query_arn, invocation_time):
        ...
        try:
            self.query_client.execute_scheduled_query(
                ScheduledQueryArn=scheduled_query_arn, InvocationTime=invocation_time
            )
    ...
    def delete_scheduled_query(self, scheduled_query_arn):
        ...
        try:
            self.query_client.delete_scheduled_query(
    ...
    def describe_scheduled_query(self, scheduled_query_arn):
        ...
        try:
            response = self.query_client.describe_scheduled_query(
                ScheduledQueryArn=scheduled_query_arn
            )
    ...
    def update_scheduled_query(self, scheduled_query_arn, state):
        ...
        try:
            self.query_client.update_scheduled_query(
                ScheduledQueryArn=scheduled_query_arn, State=state
            )
    ...
    def run(self):
        ...
        try:
            ...
            self.queue_url = timestream_dependency_helper.create_sqs_queue(
                self.QUEUE_NAME
            )
            ...
            self.subscription_arn = timestream_dependency_helper.subscribe_to_sns_topic(
                self.topic_arn, self.queue_arn
            )
            timestream_dependency_helper.set_sqs_access_policy(
                self.queue_url, self.queue_arn, self.topic_arn
            )

            # Create role for scheduled query access
            self.role_arn = timestream_dependency_helper.create_role(
                self.ROLE_NAME, self.stage, self.region
            )
            self.policy_arn = timestream_dependency_helper.create_policy(
                self.POLICY_NAME
            )
            timestream_dependency_helper.attach_policy_to_role(
                self.ROLE_NAME, self.policy_arn
            )
            ...
            self.s3_error_bucket = timestream_dependency_helper.create_s3_bucket(
                self.ERROR_BUCKET_NAME
            )

            # Scheduled Query Activities
            if self.fail_on_execution:
                self.scheduled_query_arn = self.create_invalid_scheduled_query(
                    self.topic_arn, self.role_arn
                )
            else:
                self.scheduled_query_arn = self.create_valid_scheduled_query(
                    self.topic_arn, self.role_arn
                )
            ...
            for _ in range(10):
                ...
                if not (
                    response == "SCHEDULED_QUERY_CREATING"
                    or response == "SCHEDULED_QUERY_CREATED"
                ):
                    break
            ...
            if response == "MANUAL_TRIGGER_SUCCESS":
                ...
                query_util.run_query(
                    "SELECT * FROM " + self.sq_database_name + "." + self.sq_table_name
                )
                ...
                print(
                    f"Failure Reason from SQS Notification:: {schedule_query_run_summary['failureReason']}"
                )

                if "errorReportLocation" in schedule_query_run_summary:
                    error_report_location = schedule_query_run_summary[
                        "errorReportLocation"
                    ]
                ...
                print(
                    "UNEXPECTED ERROR: Expected notification MANUAL_TRIGGER_SUCCESS, Received ",
                    response,
                )

            self.list_scheduled_queries()
            self.update_scheduled_query(
                self.scheduled_query_arn, self.SQ_DISABLED_STATE
            )
        finally:
